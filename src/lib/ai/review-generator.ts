...
async function generatePersonalizedReview(
  ...
  if (!config) {
    return buildWeeklyReviewFallback(input);
  }
        ...
        messages: [
          {
            role: "system",
            content:
              "你是面向中国大学生的成长教练，语气克制、具体、可执行。只输出合法 JSON 对象，不要 markdown。字段：summary（一段中文周报摘要）、advice（1-3 条可执行建议，合并为一段文字）、highlights（字符串数组，3-6 条要点，每条不超过 40 字）。根据数据差异给出不同角度的观察，避免套话。",
          },
